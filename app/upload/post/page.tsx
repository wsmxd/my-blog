...
const MAX_FILES = 10;
const POST_UPLOAD_TOKEN_KEY = 'postUploadToken';
...
export default function PostUploadPage() {
  ...
  useEffect(() => {
    const savedToken = localStorage.getItem(POST_UPLOAD_TOKEN_KEY);
    if (savedToken) {
      setToken(savedToken);
    }
  }, []);
    ...
    <main style={styles.page}>
      <section style={styles.card}>
        <header style={styles.header}>
          <div>
            <p style={styles.kicker}>文章配图</p>
            <h1 style={styles.title}>上传文章图片</h1>
            <p style={styles.subtitle}>这里的资源会进入独立的 post 前缀，不会出现在图床图库里。</p>
          </div>
        </header>
        ...
        <form
          ...
          onSubmit={async (event) => {
            ...
            setIsUploading(true);
            try {
              localStorage.setItem(POST_UPLOAD_TOKEN_KEY, token.trim());

              const uploads = selectedFiles.map(async (file) => {
                const response = await fetch(
                  `/api/post-images/upload?filename=${encodeURIComponent(file.name)}`,
                  {
                    method: 'POST',
                    body: file,
        ...
        >
          <label style={styles.inputLabel}>
            <span style={styles.labelText}>文章图片访问令牌 (Token)</span>
            <input
              type="password"
              value={token}
